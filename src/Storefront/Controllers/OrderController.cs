    ...
    public class OrderController : BaseController
    {
       ...
       // [Filters.WebApi.PortalAuthorize(UserRole = UserRole.None)]
        ...
        public async Task<string> PrepareOrderForUnAuthenticatedCustomer([FromBody]OrderViewModel orderDetails)
        {
            ...
            OrderNormalizer orderNormalizer = new OrderNormalizer(ApplicationDomain.Instance, orderDetails);
            orderDetails = await orderNormalizer.NormalizePurchaseSubscriptionOrderAsync().ConfigureAwait(false);
            OrderView = orderDetails;

            BrandingConfiguration branding = await ApplicationDomain.Instance.PortalBranding.RetrieveAsync().ConfigureAwait(false);

            CustomerViewModel customerRegistrationInfoPersisted = await ApplicationDomain.Instance.CustomerRegistrationRepository.RetrieveAsync(orderDetails.CustomerId).ConfigureAwait(false);
           ...
           string newCustomerId = newCustomer.CompanyProfile.TenantId;
            ...
            IPaymentGateway paymentGateway = await CreatePaymentGatewayAdmin("ProcessingOrder").ConfigureAwait(false);
            //PaymentGatewayConfig.GetPaymentGatewayInstance(ApplicationDomain.Instance, "ProcessingOrder");
            //OrderViewModel  orderToProcess =
            //    await paymentGateway.GetOrderDetailsFromPaymentAsync(payerId, paymentId, string.Empty, customerId).ConfigureAwait(false);
            ...
            await commerceOperation.PurchaseAsync(orderDetails).ConfigureAwait(false);


            //  string redirectUrl; 
            // IPaymentGateway paymentGateway;
            //if (Principal != null && Principal.IsPortalAdmin)
            //{
            //     // prepare the redirect url so that client can redirect to payment gateway.             
            string redirectUrl = string.Format(CultureInfo.InvariantCulture, "{0}/#ProcessOrder?ret=true&customerId={1}&newCustomerId={2}", Request.RequestUri.GetLeftPart(UriPartial.Authority),orderDetails.CustomerId,newCustomerId);

            // execute to preapproved action URI. 
            // paymentGateway = CreatePaymentGatewayAdmin( Resources.NewPurchaseOperationCaption);
            //}
            //else
            //{
            //    // prepare the redirect url so that client can redirect to PayPal.             
            //    redirectUrl = string.Format(CultureInfo.InvariantCulture, "{0}/#ProcessOrder?ret=true&customerId={1}", Request.RequestUri.GetLeftPart(UriPartial.Authority), orderDetails.CustomerId);

            //    // execute to paypal and get paypal action URI. 
            //    paymentGateway = PaymentGatewayConfig.GetPaymentGatewayInstance(ApplicationDomain.Instance, Resources.NewPurchaseOperationCaption);

            //}

            //string generatedUri = await paymentGateway.GeneratePaymentUriAsync(redirectUrl, orderDetails).ConfigureAwait(false);
            string appReturnUrl = redirectUrl + string.Format(CultureInfo.InvariantCulture, "&payment=success&PayerID=PayId&paymentId=preapproved"/*, orderDetails.OrderId*/);
      ...
      //  [Filters.WebApi.PortalAuthorize(UserRole = UserRole.None)]
        ...
        public async Task<SubscriptionsSummary> ProcessOrderForUnAuthenticatedCustomer(string customerId, string newCustomerId, string paymentId, string payerId)
        {
            DateTime startTime = DateTime.Now;
            ...
            // get customer
            newCustomer = await ApplicationDomain.Instance.PartnerCenterClient.Customers.ById(newCustomerId).GetAsync().ConfigureAwait(false);
            ...
            //  newCustomerId = newCustomer.CompanyProfile.TenantId;
            ...
            //CommerceOperations commerceOperation = new CommerceOperations(ApplicationDomain.Instance, newCustomerId, paymentGateway);
            //await commerceOperation.PurchaseAsync(OrderView).ConfigureAwait(false);
            ...
            // Capture the request for the customer summary for analysis.
            Dictionary<string, string> eventProperties = new Dictionary<string, string> { { "CustomerId", newCustomerId /*orderToProcess.CustomerId*/ } };
        ...
        private static async Task<IPaymentGateway> CreatePaymentGatewayAdmin(string operationDescription)
        {
            operationDescription.AssertNotEmpty(nameof(operationDescription));
           // customerId.AssertNotEmpty(nameof(customerId));
