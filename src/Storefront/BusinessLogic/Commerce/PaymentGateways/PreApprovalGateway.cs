...
namespace Microsoft.Store.PartnerCenter.Storefront.BusinessLogic.Commerce.PaymentGateways
{
    ...
    public class PreApprovalGateway : DomainObject, IPaymentGateway
    {
        ...
        //public async Task<string> GeneratePaymentUriAppAsync(string returnUrl, OrderViewModel order)
        //{
        //    CustomerSubscriptionEntity customerSubscriptions = null;
        //    foreach (var item in order.Subscriptions)
        //    {
        //        customerSubscriptions = new CustomerSubscriptionEntity(order.CustomerId, item.SubscriptionId, item.PartnerOfferId, item.SubscriptionExpiryDate);
        ...
        //    // will essentially return the returnUrl as is with additional decorations. 
        //    // persist the order. 
        //    CustomerSubscriptionEntity subDetails = await ApplicationDomain.Instance.CustomerSubscriptionsRepository.AddAsync(customerSubscriptions).ConfigureAwait(false);
