...
using Microsoft.Store.PartnerCenter.Storefront.Exceptions;
...
namespace Microsoft.Store.PartnerCenter.Storefront.Security
{
    ...
    public sealed class PartnerServiceClient : ServiceClient<PartnerServiceClient>
    {
        ...
        public async Task<AuthenticationResults> RefreshAccessTokenAsync(string authority, string resource, string refreshToken, string clientId, string clientSecret = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            try
            {
                using (HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, new Uri(authority)))
                {
                    ...
                    return new AuthenticationResults(authResponse);
                }
            }
            finally
            {
